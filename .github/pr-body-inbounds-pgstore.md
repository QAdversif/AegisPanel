...
- `migrations/0007_inbounds_params_nullable.sql` — drops the `NOT NULL` constraint on `inbounds.params`. The Go model's `Params` field has the JSON tag `json:"params,omitempty"`, which distinguishes "operator has not configured params" (nil map, field absent in the wire format) from "operator has explicitly configured an empty object" (empty map, field present with value `{}`). The original schema (`NOT NULL DEFAULT '{}'::JSONB`) was lossy on this distinction; the migration brings the constraint in line with the model's `omitempty` semantic. The DEFAULT is preserved, so existing INSERTs that omit the column still produce `'{}'::JSONB`. The only behaviour change: an explicit `params = $8` with `nil` (SQL NULL) is now accepted by the constraint.
