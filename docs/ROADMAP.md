...
| `v0.8.x`             | host → node mapping in Builder filter (**shipped this PR**: `internal/hosts.Service.HostsForInbound` + `NodesForHost` + `LookupHostForInbound` interface + `InboundSpec.HostID` populate + `users.Service.WithHosts` + v0.7.x misimplementation fix on the user-fan-out side); inbound-templates work (per-tenant `Params` defaults); UX follow-ups (merged "Add node + Provision" dialog, shadcn-vue `RadioGroup` primitive); operations polish (pre-existing eslint warnings cleanup as a `chore` PR; cosign re-signing on every release moved to v0.8.9) | ⏳ (host→node mapping done; the rest still open) |
