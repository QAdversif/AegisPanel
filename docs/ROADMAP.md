...
| `v0.8.x`             | host → node mapping in Builder filter; inbound-templates work (per-tenant `Params` defaults); UX follow-ups (merged "Add node + Provision" dialog, shadcn-vue `RadioGroup` primitive); operations polish (pre-existing eslint warnings cleanup as a `chore` PR; cosign re-signing on every release moved to v0.8.9) | ⏳ |
