...
| `v0.8.x`             | host → node mapping in Builder filter (**shipped this PR**: `internal/hosts.Service.HostsForInbound` + `NodesForHost` + `LookupHostForInbound` interface + `InboundSpec.HostID` populate + `users.Service.WithHosts` + v0.7.x misimplementation fix on the user-fan-out side); subscription URL display in UsersView (**shipped this PR**: new DropdownMenu item + new dialog with copy / open / format-preview actions, no backend change; uses the existing `/api/v1/sub/{token}` + `/api/v1/panelcfg/` endpoints; dead `fetchSubscriptionForUser` helper removed); per-user credential filter in Builder (**shipped this PR**: `internal/users.Service.AllowedUsersForNode` + `internal/cores/builder.ListUsersAllowedForNode` interface + per-inbound filter inside `BuildCoreConfigForNode` + new `usersSrc` arg on `NewFlushFn` + `a.Users` wired in `cmd/aegis/main.go`; closes the second half of the v0.7.x Phase 2 multi-user TODO, see `KNOWN_LIMITATIONS.md` "Per-user credential filter in the Builder — closed in this PR"); merged "Add node + Provision" dialog (**shipped this PR**: the v0.8.11 two-step flow becomes a single form with a "Provision this node after registering" checkbox + auth-method radio + key / password / ssh_user / ssh_port / tofu_policy / expected_fingerprint fields; the submit handler calls `createNode` then optionally `provisionNode`; the per-row "Provision" dropdown entry stays for re-provisioning offline nodes); operations polish (pre-existing eslint warnings cleanup as a `chore` PR — PR #200 shipped; cosign re-signing on every release moved to v0.8.9); shadcn-vue `RadioGroup` primitive (PR #202 — `components/ui/RadioGroup.vue` + `RadioGroupItem.vue` thin wrappers over `radix-vue`; both auth-method pickers in `NodesView.vue` migrated; hand-rolled `.nodes__auth-radios*` CSS deleted); inbound-templates work (per-tenant `Params` defaults, **PR #205 foundation shipped post-v0.8.12** — `migrations/0021_inbound_templates.sql` adds the `inbound_templates` table + `inbounds.template_id` nullable FK; new `internal/inboundtemplates/` package (model + store + pg_store + service + handler + 8 unit + 4 pg integration tests); 3 new webhook event types `inbound_template.{created,updated,deleted}`; inbounds model + `CreateInput` + `UpdateInput` + JSON request shapes gain `TemplateID *uuid.UUID`; `app.go` + `router.go` wire the new service into the App + the `/api/v1/inbound-templates` mount + the `ScopeNodes` guard; storage backend is shared with `Inbounds` via `AEGIS_INBOUNDS_BACKEND`, no new env var; the sing-box renderer's `BuildCoreConfigForNode` reading `template.params` when `inbound.template_id` is set is the follow-up PR — the actual feature — until that lands the new `template_id` column is stored but the rendered config still uses the inline `inbound.params`) | ⏳ (host→node mapping + subscription URL + per-user credential filter + merged Add+Provision + eslint cleanup + shadcn-vue `RadioGroup` primitive + inbound-templates foundation done; inbound-templates renderer integration + inbounds validation + frontend UI still open) |
