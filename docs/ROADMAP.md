...
| `v0.8.2`             | Server-side `auth.me` fix on pg backend (add `GetByID` to `auth.Store` interface, implement in `MemoryStore` and `PgStore`, rewire `Service.lookupByID`); HTTP admin surface for `user_inbound_credentials` (`/api/v1/credentials/` mount + `ScopeCredentials` + OpenAPI + Credentials tab in the user detail page) | ⏳ |
| `v0.8.x`             | `BatchedApplier` decrypt-and-use of `nodes.ssh_private_key_ciphertext` for POST /v1/apply; re-provision path for v0.3.0..v0.7.x nodes (CLI `aegis admin node rotate-panel-key <id>` + admin UI button); host → node mapping in Builder filter; inbound-templates work (per-tenant `Params` defaults); UX follow-ups (merged "Add node + Provision" dialog, shadcn-vue `RadioGroup` primitive, "show me stored public key" debug surface with fingerprint viewer); operations polish (JSON logs in production via `AEGIS_ENV=production`, cosign re-signing on every release, pre-existing eslint warnings cleanup as a `chore` PR) | ⏳ |
