...
### Verification
v0.8.23's live smoke test on aibeg.click:
...
- **Bootstrap: pin `HostKeyAlgorithms` to
  ...
  pinned. The Demo-нода (`cdn2ne.aibeg.click`)
...
### Verification
v0.8.22's live smoke test on aibeg.click:
`POST /api/v1/nodes/9ded165d-7ef1-427c-b5f2-41483c10df7b/provision`
with `tofu_policy: accept-and-append` and
...
### Verification
v0.8.21's live smoke test on aibeg.click:
`POST /api/v1/nodes/9ded165d-7ef1-427c-b5f2-41483c10df7b/provision`
...
### Verification
v0.8.20's live smoke test on aibeg.click:
`POST /api/v1/nodes/9ded165d-7ef1-427c-b5f2-41483c10df7b/provision`
with `tofu_policy: accept-and-append` and
the Demo-нода's `expected_fingerprint`
should return 200 with `state: "online"`
...
### Verification
v0.8.19's live smoke test on aibeg.click:
...
- **Provision: `joinHostPort` handles the case
  ...
  `Address = "cdn2ne.aibeg.click:22"`. The
  ...
  `"cdn2ne.aibeg.click:22:22"` (two colons).
  `dialer.DialContext("tcp", "cdn2ne.aibeg.click:22:22")`
...
### Operational
- **Smoke test on aibeg.click after deploy:** `POST /api/v1/backups/`
