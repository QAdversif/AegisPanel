...
-- backfill step.
--
-- v0.8.32.2 (#307): the file previously had no
-- `-- +migrate Up` / `-- +migrate Down` markers —
-- Up worked only via the migrator's whole-file
-- fallback (the pre-fix `upDownBodyOf` would have
-- returned the whole file as the Up body, which
-- happened to work because there was a single
-- `ALTER TABLE`), and Down silently committed an
-- empty transaction and reported success. The
-- markers below put 0013 on the same shape as every
-- other migration in the tree.
ALTER TABLE nodes ADD COLUMN agent_bearer TEXT NOT NULL DEFAULT '';

-- +migrate Down

ALTER TABLE nodes DROP COLUMN IF EXISTS agent_bearer;
