...
func TestPgDumpArgs(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			dsn:     "postgres://aegis:***REMOVED***@aegis-postgres:5432/aegis?sslmode=disable",
		...
		},
		{
			...
			dsn:     "host=aegis-postgres port=5432 user=aegis password=***REMOVED*** dbname=aegis sslmode=disable",
			want:    []string{"-Fc", "--dbname=host=aegis-postgres port=5432 user=aegis password=***REMOVED*** dbname=aegis sslmode=disable", "--no-password"},
			wantPgp: "",
		},
		{
